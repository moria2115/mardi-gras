...
function galleryCardsBuilder(img, index) {
    return `<div class="col-md-3 mt-2"  data-toggle="modal" data-target="#modal" onClick="showImages(${index})">
...
function getGalleryCard() {
    galleryImages.forEach((card, index) => {
        document.getElementById('gallery').innerHTML += galleryCardsBuilder(card, index);
    });
};
